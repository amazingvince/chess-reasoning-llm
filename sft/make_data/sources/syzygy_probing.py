...
def best_dtz_move(tablebase, board: chess.Board) -> str | None:
    ...
    for move in board.legal_moves:
        ...
        uci = move.uci()
        if our_dtz > 0:
            wins.append((uci, our_dtz))
        elif our_dtz == 0:
            draws.append(uci)
        else:
            losses.append((uci, our_dtz))

    if wins:
        # Fastest win: smallest positive DTZ
        return min(wins, key=lambda x: x[1])[0]
    if draws:
        return draws[0]
    if losses:
        # Slowest loss: most negative DTZ (largest absolute value)
