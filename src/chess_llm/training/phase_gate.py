...
def check_phase_criteria(
    ...
    *,
    phase: str | None = None,
    has_acpl: bool = False,
    allow_missing_criteria: bool = False,
) -> int:
    ...
    allow_missing_criteria : bool
        Treat missing split/metric criteria as skipped. This is useful for
        soft-gated smoke evals that intentionally cap examples per split.
    """
    print(f"\n{'=' * 60}")
    ...
    criteria = [
        ...
        ("perception", "state_tracking", 0.80, "State tracking (1 move) > 80%", "A"),
    ...
    for split, metric, threshold, label, phase_tag in criteria:
        ...
        if split not in split_results:
            if phase and not allow_missing_criteria:
        ...
        for split_name in split_names:
            ...
            for metric_name, value in split_results[split_name].items():
                if metric_name in ("overall", "acpl") or not isinstance(value, float):
                    continue
                if metric_name in DIAGNOSTIC_METRICS:
                    continue
